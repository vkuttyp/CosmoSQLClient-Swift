...
public actor MSSQLConnectionPool: SQLDatabase {
    ...
    public nonisolated var advanced: any AdvancedSQLDatabase { MSSQLConnectionPoolAdvanced(pool: self) }

    public init(configuration: MSSQLConnection.Configuration, maxConnections: Int = 10, eventLoopGroup: any EventLoopGroup = MultiThreadedEventLoopGroup.singleton) {
        ...
        if configuration.tls != .disable {
            ...
            if configuration.trustServerCertificate { tlsConfig.certificateVerification = .none }
    ...
    @discardableResult
    public func withConnection<T: Sendable>(_ work: @Sendable (MSSQLConnection) async throws -> T) async throws -> T {
    ...
    private func openConnection() async throws -> MSSQLConnection {
        try await MSSQLConnection.connect(configuration: configuration, eventLoopGroup: eventLoopGroup, sslContext: sslContext)
    }
}

struct MSSQLConnectionPoolAdvanced: AdvancedSQLDatabase {
