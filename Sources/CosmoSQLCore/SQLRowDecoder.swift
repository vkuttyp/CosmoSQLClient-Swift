...
public struct SQLRowDecoder {
    ...
    /// Decode a raw cell array into `T`, using a pre-computed column index (fast path from SQLDataTable).
    func decode<T: Decodable>(
        _ type: T.Type = T.self,
        cells: [SQLCellValue],
        columns: [SQLDataColumn],
        columnIndex: [String: Int]
    ) throws -> T {
        let decoder = _CellDecoder(
            cells: cells, columns: columns,
            columnIndex: columnIndex, strategy: keyDecodingStrategy
        )
        return try T(from: decoder)
    }
}

// MARK: - Fast cell decoder (used by SQLDataTable.decode — no SQLRow round-trip)

private final class _CellDecoder: Decoder {
    let cells: [SQLCellValue]
    let columns: [SQLDataColumn]
    /// Pre-computed lowercased column name → index map, shared across all rows.
    let columnIndex: [String: Int]
    let strategy: SQLRowDecoder.KeyDecodingStrategy
    var codingPath: [any CodingKey] = []
    var userInfo: [CodingUserInfoKey: Any] = [:]

    init(cells: [SQLCellValue], columns: [SQLDataColumn],
         columnIndex: [String: Int], strategy: SQLRowDecoder.KeyDecodingStrategy) {
        self.cells       = cells
    ...
    func container<Key: CodingKey>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        KeyedDecodingContainer(_CellKeyedContainer(decoder: self))
    }
    ...
    /// O(1) lookup using pre-built index; no per-call lowercased() heap allocations.
    func value(for key: any CodingKey) -> SQLValue? {
        let candidates = nameCandidates(for: key.stringValue)
        for name in candidates {
            if let idx = columnIndex[name.lowercased()] {
                return cells[idx].sqlValue
            }
        }
        return nil
    }
...
// MARK: - Shared coercion helper (used by both _KeyedContainer and _CellKeyedContainer)

private func _coerce<T>(_ v: SQLValue, to type: T.Type) -> T? {
    let intVal: Int64? = {
        switch v {
...
// MARK: - Cell keyed container (fast path: uses pre-built column index)

private struct _CellKeyedContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {
    let decoder: _CellDecoder
    var codingPath: [any CodingKey] { decoder.codingPath }
    var allKeys: [Key] {
        decoder.columns.compactMap { Key(stringValue: $0.name) }
    }
    ...
    func decode<T: Decodable>(_ type: T.Type, forKey key: Key) throws -> T {
        ...
        if T.self == Date.self, let s = v.asString(), let d = parseDate(s) { return d as! T }
        if T.self == UUID.self, let s = v.asString(), let u = UUID(uuidString: s) { return u as! T }
        if T.self == Decimal.self {
            if case .decimal(let d) = v { return d as! T }
            if let d = v.asDouble() { return Decimal(d) as! T }
        ...
        // Nested Decodable — re-use the same fast decoder
        let child = _CellDecoder(
            cells: decoder.cells, columns: decoder.columns,
            columnIndex: decoder.columnIndex, strategy: decoder.strategy
        )
        child.codingPath = codingPath + [key]
        return try T(from: child)
    }
    ...
    private func decode<T>(key: Key) throws -> T {
        ...
        // Delegate coercion to the original _KeyedContainer logic (shared via SQLValue extensions)
        if let result = _coerce(v, to: T.self) { return result }
        throw DecodingError.typeMismatch(T.self, context(key, "Cannot convert \(v) to \(T.self)"))
    }
